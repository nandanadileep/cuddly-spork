    ...
    try {
        ...
        const userRecord = userId
            ? await prisma.user.findUnique({
                where: { id: userId },
                select: { id: true, name: true, email: true, target_role: true, phone: true, location: true, website: true, linkedin_url: true },
            })
            : userEmail
                ? await prisma.user.findUnique({
        ...
        if (!latexResponse.ok) {
            ...
            console.error('[Resume Export PDF] LaTeXLite error:', errorText)
        ...
        if (!isDuplicate) {
            await prisma.resume.create({
                data: {
                    ...
                    title: `Resume PDF - ${new Date().toISOString().slice(0, 10)}`,
        ...
        return NextResponse.json({ error: 'Failed to export resume as PDF' }, { status: 500 })
    }
}
